...
### M22 · Phase 9 — Diagram-catalog completeness + auditability *(in progress)*
- Raise the diagram catalog's audit marks (`research/diagram-catalog-audit.md`). Graded honestly: evidence-honesty **A**, coverage **A−**, but **metadata-consistency B−** and **catalog↔product fidelity C** — the Build plan over-claimed (~14 named ✅/◐ examples absent; nothing machine-checked the catalog vs the shipped corpus). **REQ:** REQ-CATALOG-CONFORMANCE (**implemented**, ADR-0027 — the manifest `examples/catalog.json` + `conformance/catalog.test.ts` make the catalog a *verified* artifact like the goldens: a build claim can never again be untrue of the corpus), REQ-CATALOG-COVERAGE (**in progress** — first batch of **10** examples shipped: safety-behaviour, reciprocal-roles, dilemma, snag, health-anxiety, ptsd-cycle, metacognitive-cas, sorc, abc, trauma-timeline, each invariant-clean; remaining: relational-field instances, ED/psychosis loops, narrative externalising, composite), REQ-CATALOG-METADATA (*planned* — one consistent per-row schema: a dedicated safety/cultural-caveat field, per-row citations + a References section, the **picture** audience profile, TOC + anchors + "see also" cross-refs, and fold items 52–59 into their families).
