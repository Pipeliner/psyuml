...
- The **◇ new-type renderers** the catalogue lists but PsyUML can't draw + an **obstacle-avoiding edge router** + **picture-profile pictographs** (gated on M19). **REQ:** REQ-NEW-DIAGRAM-TYPES (**in progress** — four ◇ shipped: the ranked **`ladder`** (ADR-0029, exposure/fear hierarchy, rungs by `properties.intensity`), the CFT **`three-circles`** (ADR-0030, threat/drive/soothing sized by `properties.weight`, contents via `containment`), the DBT **`venn`** (ADR-0031, states of mind — Reasonable ∩ Emotion = Wise Mind; two *decorative* overlapping circles + three region *labels* by `stereotype`, contents via `containment`), the ACT values **`bullseye`** (ADR-0032, life domains plotted by `properties.intensity` = on-/off-target radius; *decorative* concentric rings + *perimeter* labels; the first edge-free model), the narrative **`tree-of-life`** (ADR-0033, items bucketed into six botanical zones by `stereotype` under a *decorative* canopy/trunk/roots silhouette; the one new layout shape), the schema-therapy **`schema-grid`** (ADR-0034, Young's 18 EMS in the 5 schema domains — a column sorter with active schemas highlighted via `properties.intensity`), and the MI **`decisional-balance`** (ADR-0035, the 2×2 change/stay × benefit/cost grid — promoted from the catalogue's excluded-tables set because its crossed axes are load-bearing, shipped with the MI ambivalence caveat) — all new DiagramTypes + renderers, no schema growth, each with a plain + golden showcase example; **1 ◇ remains** — circle-of-security), REQ-EDGE-ROUTER (**implemented** — ADR-0023/0024: `EdgeRouter` interface + both backends built; ritual + decision-nav + state-map all wired (**edge↔node enforced everywhere, `EDGE_NODE_KNOWN_GAP` empty**) AND the parts-map/process-loop free edge labels de-collided via `deCollide` (**label↔label enforced everywhere, `LABEL_LABEL_KNOWN_GAP` empty**) — both honest gaps are now universal guarantees), REQ-EDGE-CROSSING (**implemented** — ADR-0025: edge↔edge crossings brought into scope — a machine-checked `crossing.test.ts` bounds non-incident crossings to a pinned per-file baseline (a new gratuitous crossing fails CI), and the 4 structural residuals (cross-ring exit chord, cross-map polarization tie) are rendered with a legible monochrome bridge/line-hop; not a planarity claim — bounded + legible, not zero), REQ-PICTURE-PICTOGRAPHS.
