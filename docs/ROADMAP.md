...
- **Goal:** give v0.2 its structural backbone — group the existing diagram types into 8 **families** and define the 3 **audience profiles** — as metadata over the model (no schema break).
...
### M21 · Phase 10 — Product depth *(landed)*
- Persisted multi-document **case file** + editor composite authoring (ADR-0019 was in-memory/render-only); **live profile + cultural-pack loading** in the editor (ADR-0009/0019); real **i18n localization** content + RTL (the architecture exists; only `en` packs do); a **provenance-disagreement narrative** (⚖ names which schools, not how; ADR-0007); **raster/print export** (PNG/PDF). **REQ:** REQ-I18N-LOCALIZATION (**implemented** — ADR-0042: the i18n architecture (REQ-I18N) is now usable — `@psyuml/model` `isRtl`/`availableLanguages`, an editor **Language** switch that flows `lang` to render + clamps to the model's real languages, and an **RTL** `dir` flip; one illustrative en·es·ar demo pack exercises it. Honest: renderer chrome stays English (only model content localizes), RTL is text-direction not mirroring, and clinical packs need professional translators — so one marked-illustrative demo, not fabricated translations), REQ-PROVENANCE-NARRATIVE (**implemented** — ADR-0041: a new optional `provenanceNote` captures HOW contested schools differ; `renderPartsMap` draws it as a footnote (the "how" behind ⚖'s "which") + in alt-text, clinician-gated, nothing merged (§G.2); a `validate` info nudge steers authors to fill it in. Free text, parts-map visible footnote; loop-map footnote a follow-up), REQ-LIVE-PROFILES (**implemented** — ADR-0040: the editor loads a JSON §K profile / cultural pack, `validateProfile` runs the §K rules incl. the §6 cultural-permission gate, and `applyProfile` applies `roleLabelsFromProfile` **only if valid** — the same `roleLabels` surface as the built-in schools; a panel surfaces the applied profile + issues + each cultural symbol's tradition/RESTRICTED/permission/attribution; an un-permitted profile is shown but never applied. Honest: 'apply' = the vocabulary, not custom glyphs (future); session-only. Try-CFT / Try-cultural-pack quick-loads + a Playwright e2e), REQ-CASE-FILE (**implemented** — ADR-0039: a pure-container `CaseFile` schema in `@psyuml/model` (`{ kind: 'case-file', meta, documents: PsyumlModel[] }`, `parseCaseFile`/`serializeCaseFile`/`caseFileFrom`; the `kind` literal discriminates it from a single model) + editor authoring — Save/Open a `.psyuml-case`, a case title, a per-view Remove; the board IS the documents, `renderComposite` unchanged; never a model-schema change. Honest: a case file is more identifying than one view — privacy posture unchanged, per document. A model round-trip test + an author→save→reopen e2e), REQ-EXPORT-RASTER (**implemented** — ADR-0038: dependency-free browser PNG export (`downloadPng` rasterizes the deterministic SVG via `<canvas>` at 2× on a white ground) + a `@media print` stylesheet that prints the diagram + its honest caption, with browser Save-as-PDF as the PDF path; the disclaimer travels onto every export; a Playwright e2e asserts a real PNG. Honest scope: PNG is browser-only, the CLI still emits SVG).

### M26 · Phase 14 — Proper layered layout & edge routing (SOTA graph-drawing) *(in progress)*
- Upgrade layout/routing toward state-of-the-art graph-drawing methods, kept deterministic + golden-stable per the cited survey (`research/layout-algorithms.md`). **REQ:** REQ-LAYERED-LAYOUT (**implemented** — ADR-0049: the decision-nav crisis chart did only Sugiyama steps 1-2 (cycle-break + longest-path) then ordered within-layer by *model order* and centred each layer alone — skipping the two SOTA quality steps. New deterministic `packages/render/layered.ts` (`layeredLayout`) implements the full pipeline: cycle removal + longest-path rank, **crossing minimisation** (median heuristic + adjacent-transpose) and **coordinate assignment** (median alignment with the VPSC `separate1D` separation — straightens parent→child edges; a convergence node now centres under its parents). Unit-tested; all corpus geometry invariants hold (decision-nav stays crossing-free); 3 decision-nav goldens regenerated. Honest: median coords not full Brandes-Köpf (noted as the upgrade); no dummy nodes for long edges; applies to the free-ordering layered diagram — `intervention-sequence` is swimlane-constrained; force/stochastic layout is rejected as unfit for golden tests). On the **routing** side the obstacle-avoiding `EdgeRouter` was already SOTA (ADR-0023/0024: deterministic orthogonal A* + an LGPL libavoid fallback, edge↔node-enforced); it now demonstrably pairs with the layered layout (e.g. the relapse-prevention skip-edge routes cleanly down the margin around the aligned column), and a new **routing-quality invariant** (`routing.test.ts`, REQ-EDGE-ROUTER) locks the router's output — every routed detour is a clean orthogonal polyline, and the corpus is proven to exercise the router.

### M25 · Phase 13 — Usability & UX polish *(in progress)*
- Continuous usability/legibility/clarity improvements on top of the shipped system. **REQ:** REQ-DEEP-LINK (**implemented** — ADR-0048: editor deep-linking fixes the broken browse → edit flow — the showcase's only path into the editor landed on the default `state-map`, not the diagram being read. A pure, node-tested `exampleKeyFromQuery` opens `?example=<key>` when it names a shipped example (else a safe fallback; only exact known keys load — no off-corpus fetch); the editor resolves it at mount and mirrors the current example into the URL (`replaceState`) so any diagram is shareable/bookmarkable, and every showcase card gains an 'Open this diagram in the editor →' deep-link. Verified end-to-end with Playwright). Plus showcase-page UX touches on the same theme — a discoverable **Print / Save-as-PDF** button (the page was designed to print but the affordance was hidden behind Ctrl+P) and a **back-to-top** control for the long page (guarded by `conformance/showcase.test.ts`).

### M24 · Phase 12 — Text-legibility + containment invariants *(landed)*
- A visual audit of the 20 showcases (the user's "check every render for any two elements overlapping") surfaced a collision class the machine invariants were blind to: the overlap invariant (ADR-0012) guards only the `data-el` node/label boxes, so **untagged chrome text** — the title, band/lane headers, per-region contents, captions, provenance footnotes, legends — could collide unseen. **REQ:** REQ-TEXT-LEGIBILITY (**implemented** — ADR-0045: `introspect.textBoxesFromSvg` reconstructs the AABB of EVERY `<text>`, and `packages/render/text-overlap.test.ts` asserts no two text boxes overlap for every example × both layers; **14 collisions across 12 files in 5 classes** were fixed at the renderer — title-band reservation (intervention-sequence), centre-badge `deCollide` obstacle (process-loop), contents/header `textLength` caps (venn, resource-anchor), footnote↔legend gap (parts-map) — 11 goldens regenerated, suite green. Honest scope: text↔text only; text over DECORATION is intentional labelling), REQ-TEXT-CONTAINMENT (**implemented** — ADR-0046: the companion guard for the OTHER class, text clipped by its OWN container shape. Fixed the clear defect (mode-map names overflowing their dominance circles) by moving the name + `dom` below the circle → a PURE dominance glyph (cleaner notation) covered by the text↔text check; `layout-quality.test.ts` invariant C asserts every centre-outside caption is DISJOINT from its node, so with invariant A every owner label is cleanly IN or OUT, never bisected. Honest scope: interior glyph-label tightness (parts-map/relational-field) is INTENDED notation within A's slop; `decision-nav`'s crisis caption is the one documented graze; text-over-decoration stays intentional), REQ-LABEL-HALO (**implemented** — ADR-0047: a legibility pass giving every label that sits over a line or decoration a white under-glyph halo (`paint-order="stroke"`) so the line never strikes through the text — previously ad hoc (4 of 8 raw edge labels; `fitText`/`wrapLabel` had none). Made the halo a first-class `TextOpts.halo` option of the shared helpers and applied it to every edge label + over-decoration node label (secure-base banners, body-map sensations, mode-map name/dom/'↑ grow'); `legibility.test.ts` asserts every edge label carries the halo (every example × both layers) so it can't regress. Purely additive — geometry unchanged, all geometry invariants untouched; 25 goldens regenerated).

### M23 · Phase 11 — Documentation consistency + drift-proofing + the printable showcase *(landed)*
- Bring **all documentation to consistency** with the system and with itself, and make it **impossible to drift again** — a one-time hand reconciliation doesn't satisfy "impossible", so the counts/lists that actually drifted are now machine-checked. Plus a polished, self-contained **HTML showcase** of worked diagrams + a usage guide that covers drawing them **on paper**, not only in the software. **REQ:** REQ-DOC-CONFORMANCE (**implemented** — ADR-0043: `conformance/docs-conformance.test.ts`, the prose analogue of the catalog↔corpus guard (ADR-0027/0028), derives the load-bearing facts from the **system** (`DiagramType.options` count + exact type list, `FAMILIES.length`, the `examples/` corpus) and asserts the live docs match — the format-reference quick-guide lists EXACTLY the renderer types; every digit-form "N diagram types/renderers" equals the renderer count; every "N families" equals the family count; one `showcase-<type>` per type. So adding the 21st type forces the row + counts or CI fails. Also reconciled the residual drift the audit found (quick-guide completed to 20 rows; v0.2 spec / roadmap-to-v1 / ROADMAP / ARCHITECTURE / render+profiles IMPACT counts + statuses brought current). Honest scope: digit-form counts only ("eight families" is unchecked prose); a `(?<![.\w])` lookbehind skips version/identifier digits; `docs/research/**` + `sdd/adr/**` out of scope by design), REQ-SHOWCASE-PAGE (**implemented** — ADR-0044: a single-source, drift-locked, self-contained `apps/web/public/showcase.html` — one worked example of every diagram type, framed as four jobs-to-be-done (read it / draw it on paper / choose the right one / use it honestly), grouped by family, with explanations, an **on-paper** drawing guide, honesty notes and a print stylesheet so it prints onto paper. Generated by `scripts/build-showcase.mjs` from `examples/showcase.json` (editorial prose only — titles come from the models, renders from the committed `showcase-<type>.svg` goldens, `family`/chooser from `FAMILY_OF`/`FAMILIES`) and drift-checked by `conformance/showcase.test.ts` (manifest vs system; generated page vs manifest). Honest: one example per type (not the 40+ catalogue); the guard locks structure, not the hand-authored wording; `pattern`/`composite` render empty and are noted honestly; output gitignored, linked from the editor).

### M22 · Diagram-catalog completeness + auditability *(landed)*
