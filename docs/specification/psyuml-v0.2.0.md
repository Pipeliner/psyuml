...
- **Tier A (automatable, enforced in CI):** the existing conformance + accessibility + **overlap invariant** (ADR-0012) + monochrome-redundancy checks. A symbol set `MUST NOT` rely on colour alone (v0.1 §D — and note PoN itself is *not* accessibility-validated, so v0.1's rule is ahead of it).
