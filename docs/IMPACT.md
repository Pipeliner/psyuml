...
| `extension-guide.md` | Practical §K companion — how to add a school/symbol via a profile + stereotypes (the four rules, semver, deprecation), the worked CFT profile, and `psyuml lint-profile` | spec §K/§B/§C/§L.2, `packages/profiles` | contributors | REQ-EXTENSION-MECH | low |
| `handbook.md` | Practitioner handbook/tutorial — five working principles, the two layers, choosing a diagram, per-diagram how-tos (when/build/in-session/pitfall/example), co-creating in session, cross-school honesty, safety+ethics in practice, exercises, and the honest limits | spec §E/§L, `packages/validate`, `packages/render`, `examples/*` | clinicians, learners | REQ-HANDBOOK | low |
...
| `evaluation-suite-pilot-2.md` | A **simulated** blind per-symbol comprehension dry-run (Instrument D / ISO 9186; 6 **no-inside-view** participant agents over `NOTATION_SYMBOLS`) — explicitly **not evidence** (LLMs can't see; tiny N); surfaced redesign candidates (`◇`-resource read as a decision diamond, `⚖`-contested read as balance, band↔barrier confusability) and a strong dual-coding signal (the way-out marker is unsafe glyph-only). Gate + ledger unchanged | `evaluation-suite.md`, `adoption/comprehension-instruments.md`, `packages/profiles` (`NOTATION_SYMBOLS`) | maintainers | REQ-EVAL-SUITE, REQ-NOTATION-TESTING | low |
| `evaluation-suite-pilot-3.md` | A **simulated** blind formulation-comprehension dry-run (Instrument A) where 4 **no-inside-view** participant agents **read the actual rendered diagrams as PNGs** (real pixels — a PNG strips the alt-text answer key) — the crisis chart, State Map, and CAT trap loop. Explicitly **not evidence** (multimodal vision ≠ human vision, likely optimistic; tiny N). Strong comprehension (crisis-chart safety items at ceiling; bare-glyph failures from pilot 2 resolved in-context → dual-coding vindicated); surfaced a State-Map **text-legibility** flag. Gate + ledger unchanged | `evaluation-suite.md`, `examples/*`, `packages/render` | maintainers | REQ-EVAL-SUITE, REQ-NOTATION-TESTING | low |
