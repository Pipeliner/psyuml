...
| `comprehension-instruments.md` | **Proposed, unvalidated** Tier-B instruments — layperson comprehension test (sample items + scoring), inter-rater formulation-reliability task, multi-school Delphi sketch, **+ Instrument D: per-symbol ISO 9186 comprehension (v0.2 §5)** with thresholds (≥67% / ≥85% safety) and a **per-symbol ledger** (every Tier-B cell `pending`; the Tier-A `auditNotation()` half is automated/green) — + a pilot-study design with go/no-go thresholds tied to the Stage-4 gate; framed as **not evidence** | spec §J/§L.2, v0.2 §5, `../evaluation-suite.md`, `../evaluation-suite-pilot.md`, `packages/profiles` (`NOTATION_SYMBOLS`), `../../examples/*` | study designers, maintainers | REQ-ADOPTION (extends REQ-EVAL-SUITE, REQ-NOTATION-TESTING) | low |
