...
| `index.ts` | `validate(model,{layer})` — well-formedness (edge endpoints, band refs, relation/barrier endpoint kinds §A.2-r3/r6, no floating intervention §A.2-r4) + path-of-hope + crisis no-dead-ends/resources + client disclaimer gate + a11y label-length + **safety triage** (acute-risk/psychosis escalation, ritual-under-psychosis contraindication) + **mixed-school provenance** awareness (diagram-level + per-node opposed-claim surfacing); exports `requiresHumanEscalation` (the AI/UI halt gate) | `@psyuml/model` | apps/web, ai (M8) | §A.2, §D, §G.2, §L.2 / REQ-WELLFORMEDNESS, REQ-PATH-OF-HOPE, REQ-SAFETY-TRIAGE, REQ-ETHICS-GUARDRAILS, REQ-CROSS-SCHOOL, REQ-ACCESSIBILITY | medium |
