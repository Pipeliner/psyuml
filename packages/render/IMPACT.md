...
| `index.ts` | Render API (`renderStateMap`, `renderPartsMap`, `renderDecisionChart`, `renderResourceMap`, `renderLoopMap`, `renderTimeline`) | model, profiles, assets | apps/web | §B–§E (state/parts/loop/timeline/decision/resource) / REQ-NOTATION, REQ-PROCESS-LOOP, REQ-TIMELINE, REQ-DECISION-NAV, REQ-RESOURCE-ANCHOR | medium |
