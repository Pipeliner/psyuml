# Impact — `packages/profiles/` (`@psyuml/profiles`)

**Purpose:** school profiles (glyph/palette + layout + vocabulary + extra validators) and the
translation table — the "many views over one model" layer. (M0: seed list; M4–M5: definitions.)
**Status:** implemented (M4–M5 — translation table + the §K extension mechanism)
**Spec anchor / REQ:** REQ-CROSS-SCHOOL, REQ-RESEARCH-PROFILES, REQ-BODY-MAP, REQ-EXTENSION-MECH

## Upstream (this depends on)
- `@psyuml/model` (NodeKind/EdgeKind/DiagramType/Tier — the core element set the §K base/compat
  checks are derived from), `zod` (the profile schema).
- `../../docs/specification/psyuml-v0.1.0.md` §G (translation), §K (extension), §B/§C (glyphs).
- `../../docs/extension-guide.md` — the practical §K companion.

## Downstream (depends on this) — blast radius
> **Blast radius: medium.** `render` consumes profile glyph/layout choices; `apps/web` shows
> the profile switcher. Provenance tags here keep opposed origin-claims visible (§G.2).

## Files
| File | Purpose | Upstream | Downstream | Spec / REQ | Change risk |
|---|---|---|---|---|---|
| `package.json` | Package manifest (`@psyuml/profiles`) | — | workspace resolution | — | low |
| `index.ts` | Profiles API: `SEED_PROFILES`, `listProfiles`, the §G.2 `TRANSLATIONS` table, `translate(concept,school)`, `roleLabelsFor(school)`; **§K extension mechanism** — `ExtensionProfile`/`StereotypeDef` schemas, `validateProfile` (4 rules + Tier-1 freeze + glyph collision + semver/deprecation, ADR-0009), `roleLabelsFromProfile`, `CORE_BASES`/`CORE_TIER1_GLYPHS`, the worked `CFT_PROFILE`; **v0.2 §6 cultural-permission** (ADR-0019) — `StereotypeDef.cultural` (tradition / restricted / permission / attribution) + the `profile.cultural-permission` rule (a closed/initiatory/restricted symbol MUST declare permission and MUST NOT be a generic icon) + the worked `CULTURAL_PACK_EXAMPLE`; **v0.2 families + audience profiles** (ADR-0014) — `FAMILIES`/`FAMILY_OF`/`familyOf`/`diagramsInFamily` (8 families over the 20 diagram types) + `AUDIENCE_PROFILES`/`audienceProfile` (clinician/client/picture) + `withinSymbolBudget(kinds, audience)` (ADR-0016 — the §2 graphic-economy cap: clinician none, client 6, picture 5; the editor surfaces it as a gentle nudge); **v0.2 notation testing** (ADR-0017, §5) — `NOTATION_SYMBOLS` (the enumerated symbol set under comprehension test) + `auditNotation()` (the **Tier-A** harness: discriminability / dual-coding of safety-critical symbols / gloss, CI-enforced — the automatable pre-study gate; the ISO 9186 **Tier-B** human study stays pending in the ledger); **picture-profile pictographs** (ADR-0037, REQ-PICTURE-PICTOGRAPHS, §1/§8) — `PICTOGRAPHS` (drawn 24×24 candidate icons, each dual-coded, all `comprehension: 'pending'`) + `auditPictographs()` (the Tier-A gate — discriminable icons, every symbol dual-coded, gloss/id, **and** the honesty gate: while `PICTOGRAPH_STUDY_RUN` is false every symbol MUST stay `pending`) + `pictographFor()` (the gated render hook — returns nothing until a symbol passes, so picture stays "client + flag", wired but inert) + `availablePictographs()` + `pictographKeySvg()` (the "NOT validated" candidate sheet the editor shows in picture mode); the Tier-B comprehension study stays the gate (REQ-STUDY-PREREG) and is never fabricated | `@psyuml/model`, zod | render (via `roleLabels`), cli (`lint-profile`), conformance, apps/web | §G, §K, v0.2 §2, §5, §1, §8 / REQ-CROSS-SCHOOL, REQ-EXTENSION-MECH, REQ-DIAGRAM-FAMILIES, REQ-AUDIENCE-PROFILES, REQ-NOTATION-TESTING, REQ-PICTURE-PICTOGRAPHS, REQ-CULTURAL-PERMISSION | medium |
| `index.test.ts` | Unit tests (seed list + translation table + the §K rule checks) | `index.ts` | CI `test` | — | low |

## Change checklist
- [ ] New profile ⇒ follow §K extension rules (map to a §A element; Tier-3; collision check).
- [ ] Never flatten opposed theoretical claims — tag provenance (§G.2 caveat).
- [ ] Ran `node sdd/check.mjs` (green).
