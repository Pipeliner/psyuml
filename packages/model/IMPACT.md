...
| `index.ts` | Model API (`createEmptyModel`, `PsyumlModel`, `getText`, `serializeModel`; `schoolClaims` — the school-agnostic provenance-claim reader shared by validate + render, ADR-0007). **v0.2 (ADR-0014/0015, additive/backward-compatible):** `EpistemicStatus` gains the §3 provenance values `jointly-agreed` / `clinician-inferred` / `contested`; `INTERPRETIVE_STATUSES` + `isInterpretive()` are the shared descriptive-vs-interpretive predicate (render + lints agree); `Properties.asIf` is the §4/§6 ontology-neutral "as-if" qualifier; `Edge.loopTopology` (`trap`/`dilemma`/`snag`) is the §4 CAT loop-shape tag. **Case file (ADR-0039, REQ-CASE-FILE):** the `CaseFile` schema — `{ version, kind: 'case-file', meta, documents: PsyumlModel[] }` — a **pure container** of several views of one case (the saved Composite board), with `parseCaseFile`/`serializeCaseFile` (lossless round-trip) + `caseFileFrom`; the `kind` literal discriminates a case file from a single model. NEVER a model-schema change — each document is an ordinary `PsyumlModel`. **Localization (ADR-0042, REQ-I18N-LOCALIZATION):** `isRtl(lang)` (RTL base subtags) + `availableLanguages(model)` (the langs a model's labels carry, `model.language` first) — make the existing concept-keyed-label i18n architecture usable (the editor's locale switch + RTL) | — | all dependents | §A, §G.2, v0.2 §3/§4, §2 / REQ-CORE-ONTOLOGY, REQ-CROSS-SCHOOL, REQ-EPISTEMIC-STATUS, REQ-PROVENANCE-CONFIDENCE, REQ-PATTERN-SEMANTICS, REQ-CASE-FILE, REQ-PROVENANCE-NARRATIVE, REQ-I18N-LOCALIZATION | **high** |
