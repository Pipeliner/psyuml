...
| `index.ts` | Model API (`createEmptyModel`, `PsyumlModel`, `getText`, `serializeModel`; `schoolClaims` — the school-agnostic provenance-claim reader shared by validate + render, ADR-0007). **v0.2 (ADR-0014/0015, additive/backward-compatible):** `EpistemicStatus` gains the §3 provenance values `jointly-agreed` / `clinician-inferred` / `contested`; `INTERPRETIVE_STATUSES` + `isInterpretive()` are the shared descriptive-vs-interpretive predicate (render + lints agree); `Properties.asIf` is the §4/§6 ontology-neutral "as-if" qualifier; `Edge.loopTopology` (`trap`/`dilemma`/`snag`) is the §4 CAT loop-shape tag | — | all dependents | §A, §G.2, v0.2 §3/§4 / REQ-CORE-ONTOLOGY, REQ-CROSS-SCHOOL, REQ-EPISTEMIC-STATUS, REQ-PROVENANCE-CONFIDENCE, REQ-PATTERN-SEMANTICS | **high** |
