...
- `@psyuml/model` (parse/serialize), `@psyuml/validate` (lint), `@psyuml/render` (render), `@psyuml/grammar` (DSL convert), `@psyuml/privacy` (redact), `@psyuml/profiles` (`lint-profile`), `@psyuml/interop` (`export` → FHIR).
...
| `index.ts` | `run(argv, io)` — `lint` (validate, exit≠0 on error), `render` (→SVG), `convert` (JSON⇄DSL), `redact` (de-identify), **`export`** (lossy FHIR R4 via `@psyuml/interop`; `--scope`, `--no-deidentify`, `--code id=system|code|display` caller-supplied terminology binding, sanity-checks its own bundle + prints the loss report + a consent warning to stderr, ADR-0018), `template` (blank printable scaffold), `lint-profile` (validate a §K extension profile JSON, ADR-0009); auto-detects JSON vs DSL input | model, validate, render, grammar, privacy, profiles, interop | bin.ts, tests | §J, §B/§C, §D, §K, v0.2 §7 / REQ-TEXT-DSL, REQ-PRIVACY, REQ-TEMPLATES, REQ-EXTENSION-MECH, REQ-INTEROP-FHIR | low |
