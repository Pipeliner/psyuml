...
| `App.tsx` | Editor: live render (all 12 diagram types, example-keyed picker), layer/monochrome/**school** toggles, **acute-risk/psychosis flags**, palette, node panel (rename/hide), text alt, formulation-health panel + export gating, **clinical-escalation banner**, **Compare-with (longitudinal diff)** panel + **in-session version snapshots** (snapshot/compare/restore), **Text (DSL) view/edit**, save/export | `@psyuml/model`, `@psyuml/render`, `@psyuml/validate`, `@psyuml/profiles`, `@psyuml/diff`, `@psyuml/grammar`, `editor.ts`, `examples/*.psyuml` | — | REQ-EDITOR-MVP, REQ-UX-STORIES, REQ-COLLAB, REQ-CLIENT-SAFETY-UX, REQ-PATH-OF-HOPE, REQ-SAFETY-TRIAGE, REQ-CROSS-SCHOOL, REQ-VERSIONING-DIFF, REQ-TEXT-DSL | medium |
| `editor.ts` | Pure editor-state helpers (`addNode`, `nextId`, `setNodeLabel`, `setNodeHidden`, `setSafetyFlag`, `snapshotModel`/`restoreVersion`) — testable in node | `@psyuml/model` | `App.tsx`, `editor.test.ts` | REQ-EDITOR-MVP, REQ-COLLAB, REQ-CLIENT-SAFETY-UX, REQ-SAFETY-TRIAGE, REQ-VERSIONING-DIFF | low |
