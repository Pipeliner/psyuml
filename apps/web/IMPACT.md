...
| `App.tsx` | Editor: live render (all 12 diagram types, example-keyed picker), layer/monochrome/**school** toggles, **acute-risk/psychosis flags**, node panel (rename / **mark certainty** / hide / **remove**), **Diagram-details editor** (title/disclaimer/crisis), text alt, formulation-health panel + export gating, **clinical-escalation banner**, **Compare-with (longitudinal diff)** + **in-session version snapshots**, **Text (DSL) view/edit**, save/export | `@psyuml/model`, `@psyuml/render`, `@psyuml/validate`, `@psyuml/profiles`, `@psyuml/diff`, `@psyuml/grammar`, `editor.ts`, `examples/*.psyuml` | — | REQ-EDITOR-MVP, REQ-UX-STORIES, REQ-COLLAB, REQ-CLIENT-SAFETY-UX, REQ-EPISTEMIC-STATUS, REQ-ETHICS-GUARDRAILS, REQ-PATH-OF-HOPE, REQ-SAFETY-TRIAGE, REQ-CROSS-SCHOOL, REQ-VERSIONING-DIFF, REQ-TEXT-DSL | medium |
| `editor.ts` | Pure editor-state helpers (`addNode`, `nextId`, `setNodeLabel`, `setNodeEpistemic`, `setNodeHidden`, `removeNode`, `setMeta`, `setSafetyFlag`, `snapshotModel`/`restoreVersion`) — testable in node | `@psyuml/model` | `App.tsx`, `editor.test.ts` | REQ-EDITOR-MVP, REQ-COLLAB, REQ-CLIENT-SAFETY-UX, REQ-EPISTEMIC-STATUS, REQ-ETHICS-GUARDRAILS, REQ-SAFETY-TRIAGE, REQ-VERSIONING-DIFF | low |
