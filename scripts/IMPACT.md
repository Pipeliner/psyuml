...
| `build-showcase.mjs` | **Generates the standalone HTML showcase** (`apps/web/public/showcase.html`, ADR-0044) from `examples/showcase.json` + the committed `examples/showcase-<type>.svg` goldens + each model's own title: one worked example of every diagram type, framed as four jobs-to-be-done (read / draw-on-paper / choose / use-honestly), grouped by family, with explanations, an on-paper drawing guide, honesty notes and a print stylesheet. Self-contained (inline CSS + inline SVG, no external assets). Runs in `dev`/`build` after build-docs.mjs; exports `renderShowcase` + `esc` for `conformance/showcase.test.ts` (drift check) — typed for that importer by the sibling `build-showcase.d.mts` (since `scripts/` is outside the tsconfig program). Output gitignored (`apps/web/public/*.html`) | `examples/showcase.json`, `examples/showcase-*.svg` | `apps/web/public/showcase.html`, `App.tsx` doc link, `conformance/showcase.test.ts` | REQ-SHOWCASE-PAGE, REQ-EXAMPLE-LIBRARY | low |
